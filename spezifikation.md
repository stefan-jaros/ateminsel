...
- **Server**: Reginas Hoststar-Konto (Plesk-Hosting)
- **Pfad**: `/ateminsel.ch/public_html/test/ateminsel/` (Testphase, vor Umstellung auf Live)
- **Deploy**: GitHub Actions → SFTP (wlixcc/SFTP-Deploy-Action)
  - Trigger: Push auf `main`, nur bei Aenderungen in `site/`
  - Manuell ausloesbar via `workflow_dispatch` (Actions → Run workflow)
  - Secrets: `FTP_HOST` (lx39.hoststar.hosting), `FTP_USER` (ftp.ateminsel.ch), `FTP_PASS`
  - Port 5544, `sftp_only: true`, `HostKeyAlgorithms=+ssh-rsa` (Hoststar nutzt aelteren Hostkey)
- **Test-URL**: `ateminsel.ch/test/ateminsel/`
- **Status**: `noindex, nofollow` (Testphase)
- Frueher: Deploy auf `jaroszewski.ch/ateminsel/` (Stefan's Server) via SamKirkland/FTP-Deploy-Action — am 2026-05-14 auf Reginas Server umgestellt.
- **Cache-Buster**: `style.css?v=13` und `main.js?v=9` in allen HTML-Dateien
...
8. **Termin buchen** – Calenso-Widget als iframe (widget.calenso.com/?partner=ateminsel-buchung), Sprachparameter pro Sprache (`de_CH`, `en_US`, `fr_CH`, `es_ES`). Eigener heller Kasten um das iframe, der auch im Dark Mode hell bleibt — Calenso ist eine fremde Seite, in deren CSS wir nicht reingreifen koennen.
